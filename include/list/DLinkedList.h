...
typename DLinkedList<T>::Node *DLinkedList<T>::getPreviousNodeOf(int index)
{
    ...
    // TODO
    if (index < 0 || index > count) throw std::out_of_range("Index is out of range!");
    if (count == 0 || index == 0) return head;
    if (index == count) return tail->prev;
    if (index <= count/2)
    {
        Node* temp = head->next;
...
bool DLinkedList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    int i = indexOf(item);
    if (i == -1) return false;
    Node* temp = getPreviousNodeOf(i);
    if (removeItemData != nullptr) removeItemData(temp->next->data);
    removeAt(i);
    return true;
}
...
string DLinkedList<T>::toString(string (*item2str)(T &))
{
    ...
    for(int i = 0; i < count; i++)
    {
        if(item2str) ss << item2str(temp->data);
        else ss << temp->data;
        if(i < count - 1)
        {
            ss << ", ";
...
void DLinkedList<T>::removeInternalData()
{
     ...
     */
    // TODO
    if (this->deleteUserData != nullptr) deleteUserData(this);
    Node* temp = head->next;
    while (temp != tail)
    {
